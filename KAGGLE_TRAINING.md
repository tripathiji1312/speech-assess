...
| `RuntimeError: Expected to mark a variable ready only once ... marked as ready twice` (Cell 4, step 0) | unsloth's patched model hardcodes reentrant checkpointing, which is incompatible with DDP on 2 GPUs. Fixed in current code: `use_gradient_checkpointing=True` (unpatch smart GC) + force `use_reentrant=False` under torchrun (same fix unsloth shipped for VLM, PR #3751); `git pull` and re-run. Quick check before the ~6h run: append `--max-steps 3 --logging-steps 1` to the Cell 4 command — expect 3 loss lines, no crash (~10 min). If it then OOMs, use `--batch-size 2 --grad-accum 4` |
